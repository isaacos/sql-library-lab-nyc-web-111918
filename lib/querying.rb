...
def select_series_title_with_most_human_characters
  ...
  JOIN characters ON series.id = characters.series_id
   GROUP BY species ORDER BY COUNT(*) LIMIT 1;"
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(character_id) FROM characters
   JOIN character_books on characters.id = character_books.character_id GROUP BY characters.name ORDER BY COUNT(character_id) DESC;"
end
